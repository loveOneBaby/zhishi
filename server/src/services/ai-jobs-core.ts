...
export function trimJobOutput(value: string): string {
...
export async function persistJobNow(job: AiKnowledgeBaseJob): Promise<void> {
...
export async function persistJob(job: AiKnowledgeBaseJob): Promise<void> {
  discardedJobIds.delete(job.id);
  await persistJobNow(job);
}

export async function touchJob(job: AiKnowledgeBaseJob, options: { immediate?: boolean } = {}): Promise<void> {
...
export function pushJobLog(job: AiKnowledgeBaseJob, message: string): Promise<void> {
...
export function resetJobStats(job: AiKnowledgeBaseJob): void {
...
export function recordUsage(job: AiKnowledgeBaseJob, usage: AiTokenUsage): Promise<void> {
...
export function finishJobTimer(job: AiKnowledgeBaseJob): void {
...
// 任务执行器注册表：各 runner 文件（ai-jobs-runners/*）import 本模块后自注册到这张表，
// runQueuedJob 按 kind 分发。这样本模块无需直接 import runner（会形成循环依赖），
// runner 反过来 import 本模块的共享 helper / 状态即可。
const jobRunners = new Map<AiKnowledgeBaseJob['kind'], (job: AiKnowledgeBaseJob) => Promise<void>>();
export function registerJobRunner(kind: AiKnowledgeBaseJob['kind'], runner: (job: AiKnowledgeBaseJob) => Promise<void>): void {
  jobRunners.set(kind, runner);
}
function runQueuedJob(job: AiKnowledgeBaseJob): Promise<void> {
  const runner = jobRunners.get(job.kind);
  if (!runner) throw new Error(`未注册的 AI 任务类型：${job.kind}`);
  return runner(job);
}
...
export async function hydrateKnowledgeBaseResult(job: AiKnowledgeBaseJob): Promise<GeneratedKnowledgeBaseResult | null> {
...
export function extractPlanFromOutput(job: AiKnowledgeBaseJob): GeneratedKbDraft | null {
...
export function isGeneratedKbDraft(value: unknown): value is GeneratedKbDraft {
...
export function folderPathLabelFromSnapshot(folderId: string, foldersById: Map<string, Folder>): string {
  const names: string[] = [];
  const seen = new Set<string>();
...
export async function updateJobResult(job: AiKnowledgeBaseJob, result: GeneratedKnowledgeBaseResult): Promise<void> {
  job.result = {
    kb: result.kb,
    folders: [...result.folders],
    entries: [...result.entries],
...
export function mergedTags(primary: string[] | undefined, fallback: string[] | undefined): string[] {
  const out: string[] = [];
...
export async function stopJobForDeletedKb(job: AiKnowledgeBaseJob): Promise<void> {
...
export function agentEditCounts(actions: AgentEditAction[]): { structure: number; content: number } {
  return {
...
export function planFromJob(job: AiKnowledgeBaseJob): AgentEditPlan | null {
...
export async function findExistingFolder(kbId: string, name: string, parentId: string | null): Promise<Folder | null> {
  const target = name.trim();
  if (!target) return null;
  return (await listFolders()).find((folder) => folder.kbId === kbId && folder.parentId === parentId && folder.name === target) ?? null;
}

export async function resolveAgentActionFolder(
  kbId: string,
...
export async function refreshAgentEditResult(job: AiKnowledgeBaseJob): Promise<void> {
