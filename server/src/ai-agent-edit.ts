...
- 由你根据用户语义判断生成 1 条还是多条 create-entry：明确单一主题时生成 1 条；表达批量补充、场景扩展、题库补全、多个方向时拆成多条；用户写明数量时按数量规划，最多 8 条。
- 多条知识点必须每条独立成题，不要合并成一个综合知识点；场景题 topic 要以具体场景或问题表达，例如“什么时候需要用 X？怎么判断？”“X 效果变差怎么排查？”“X 和 Y 怎么选型？”“X 在线上怎么监控和优化？”。
