...
export async function analyzeKnowledgeBase(kbId: string, signal?: AbortSignal): Promise<KbAnalysis> {
  const kb = getKb(kbId);
  if (!kb) throw new Error('知识库不存在');
  const folders = (listFolders() as FolderLike[]).filter((f) => f.kbId === kbId);
  const entries = (listEntries() as unknown as EntryLike[]).filter((e) => e.kbId === kbId);
...
export async function analyzeEntry(entryId: string, signal?: AbortSignal): Promise<KbAnalysis> {
