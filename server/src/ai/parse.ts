...
export function coerceGeneratedDraft(raw: unknown, topic: string): GeneratedDraft {
  ...
  return {
    ...
    tags,
    sections,
    flowchart: text(obj.flowchart) || text(obj.asciiFlowchart) || text(obj.answerFlowchart),
...
export function coerceGeneratedKbDraft(raw: unknown, domain: string): GeneratedKbDraft {
  ...
  const questions = rawQuestions.map((question, index) => {
    ...
    return {
      ...
      answer,
      flowchart: text(item.flowchart) || text(item.asciiFlowchart) || text(item.answerFlowchart) || undefined,
