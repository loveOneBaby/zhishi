...
当前 DMG 面向 Apple 芯片 Mac 构建（arm64）。打开 DMG 后双击「知识检索.app」，应用会自动复制到 Applications 并重新打开。
...
桌面端启动后会自动检查 GitHub Release 最新版本；也可以在菜单中点击「检查更新...」。检测到新版本后，主页面右上角会出现更新按钮；点击后开始下载并显示进度，下载完成后按钮会变成「安装」。点击安装时应用会先询问用户，确认后退出，在后台完成安装，并在完成后自动重新打开。

早期版本使用 ad-hoc 签名，macOS Squirrel 自动安装会因为每次打包的 `cdhash` 不同而报“代码未能满足指定的代码要求”。修复该问题的版本需要手动安装一次；之后更新按钮会使用应用内安装器解压已校验的 zip、替换 Applications 中的应用，并自动重新打开。

注意：如果 GitHub 仓库是 private，安装后的桌面应用无法携带你的 GitHub 登录态读取 Release 更新文件，自动更新会拿到 404，因此不会弹出新版本提示。要让自动更新对普通安装包生效，需要把 Release 资源放在公开可访问的位置，例如公开仓库、公开 GitHub Pages、对象存储或自建更新服务。
